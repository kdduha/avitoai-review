...
допускается. Причина не в недоверии к арифметике как таковой, а в том, что
балл должен быть воспроизводимым: два прогона одной работы обязаны дать один
и тот же итог, а объяснение «почему 62, а не 65» должно собираться из
слагаемых, а не пересказываться моделью.

Порядок операций зафиксирован и важен:

    сумма с весами → округление по шагу шкалы → порог зачёта
    → обязательные минимумы по критериям → штраф за просрочку

Штраф последний, потому что по условиям курсов он применяется к итоговой
оценке, а не к отдельным критериям. Обязательные минимумы стоят до штрафа,
потому что провал по обязательному критерию — это незачёт по существу
работы, и просрочка тут ни при чём.
"""
    ...
    *,
    ...
    for verdict in verdicts:
        ...
        if criterion is None:
            # Модель вернула критерий, которого нет в рубрике. В сумму он не
            # идёт, но и молча пропадать не должен.
        ...
        minimum = getattr(criterion, "min_score_for_pass", None)
...
def _decide(breakdown: ScoreBreakdown, rubric: Any) -> tuple[bool, str]:
    ...
    threshold = getattr(rubric.scale, "pass_threshold", None)
