...
class ReviewConfig:
    ...
    """Критерии с auto_verifiable проверены Format Gate детерминированно.

    По умолчанию всё равно отдаём их модели: гейт отвечает на вопрос «есть
    ли», а критерий — на вопрос «насколько хорошо». Флаг оставлен для
    экономии там, где этой разницы нет.
    """


class ReviewService:
    def __init__(self, gateway: PrivacyGateway, config: ReviewConfig | None = None) -> None:
    ...
    def review(
        ...
        *,
        ...
        criteria = [
            ...
            if not (self.config.skip_auto_verifiable and getattr(c, "auto_verifiable", False))
        ]

        draft = ReviewDraft(
            assignment_id=rubric.assignment_id,
            rubric_title=getattr(rubric, "title", ""),
            max_score=rubric.scale.total_max,
            gate_facts=list(gate_facts or []),
        )

        for batch in batch_criteria(criteria, self.config.batch_size):
            verdicts = self._review_batch(
                batch, rubric, artifacts, gate_facts or [], condition_text, draft
            )
        ...
        breakdown = aggregate(
            ...
            submitted_at=getattr(bundle, "submitted_at", None),
            deadline_at=getattr(bundle, "deadline_at", None),
        )
    ...
    def _review_batch(
        ...
        messages = build_messages(
            rubric, batch, artifacts,
            gate_facts=gate_facts, condition_text=condition_text,
        )
        try:
            result, _ = complete_json(
                self.gateway, messages, CriterionBatch,
    ...
    def _fill_missing(self, draft: ReviewDraft, criteria: list[Any]) -> None:
