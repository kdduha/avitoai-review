...
Разбор репозитория примеров (§5.5): в «хорошем решении» курса LLM лежит
`mlruns/` на 314 файлов трекинга MLflow. Это не работа, это след запуска. Туда
же `go.sum`, `__pycache__`, локфайлы. Каждый артефакт получает класс, и от
класса зависит, куда он попадёт:

- `solution` — идёт в ревью и в детектор;
- `evidence` — графики и скриншоты: учитываются по факту наличия, текстом их
  не читают;
- `tooling` — `Dockerfile`, CI, миграции, сгенерированные `*.pb.go`. **Из
  детектора исключается тоже**: сгенерированный фреймворком код ничего не
  говорит о самостоятельности студента, а детектор без этого ловит сам себя;
- `noise` — не идёт никуда.

Правило жило в трёх местах сразу и в каждом по-своему: денилист в конфиге
ingest, список шаблонов в судье детектора и своя разметка в демо-скрипте.
Расхождение было не теоретическим — боевой провайдер размечал роли беднее, чем
демо: `tooling` он не ставил вовсе, и весь служебный код уходил в детектор как
работа студента.
"""
