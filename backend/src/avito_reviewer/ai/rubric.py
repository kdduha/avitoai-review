...
Промпт генерируется из этой структуры, а не пишется под задание: добавить курс
значит добавить JSON. Схема вмещает все формы, в которых условия приходят на
самом деле — таблицу с баллами, N вопросов по фиксированному баллу, чек-лист без
баллов вовсе, требования к формату со штрафами.

Три поля объясняют, почему оценка не сводится к сумме:

* `min_score_for_pass` — провал по обязательному критерию не компенсируется
  набранным на остальных;
* `step` — баллы дробные, округлять надо по правилу задания;
* `checks[]` — проверочные пункты из условия; модель отвечает по каждому
  отдельно, и разброс между прогонами резко падает.

Здесь только схема, разбор и реестр; заполняет рубрику методист.
"""
...
def validate_rubric(rubric: Rubric) -> list[str]:
    ...
    # Веса позволяют сумме не совпадать с максимумом; недостижимый максимум — нет.
