...
def extract_json(text: str) -> str:
    ...
    # Только ограда вокруг всего ответа: работа по системному дизайну состоит из
    # блоков ```mermaid, и поиск ``` по всему тексту вырезал бы диаграмму.
...
def complete_json(
    ...
    *,
    ...
    # `max_tokens` четырёхсоткой, а 4xx не ретраится.
    ...
    while True:
        ...
        # Не ошибка форматирования, а нехватка места; пустой ответ рассуждающей
        # модели отсюда же — бюджет ушёл в `reasoning`, а `finish_reason` бывает
        # штатным. Просить «верни валидный JSON» бессмысленно: тот же потолок.
        ...
        if needs_room and budget_retries and budget < ceiling:
            ...
            budget = grown
            # Попытка добыть места не тратит попытку починки: разные неисправности.
            continue
