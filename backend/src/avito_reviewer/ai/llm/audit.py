...
# Открытые прогоны: пары (журнал, приёмник). Именно ContextVar, а не глобальный
# список — параллельные запросы не должны видеть записи друг друга.
...
class AuditLog:
    ...
    def collect(self) -> Iterator[list[AuditRecord]]:
        ...
        Журнал один на приложение — иначе не сложить стоимость прогона курса, — но
        черновик обязан отчитаться о своих токенах. Срезом по длине это не
        решается: запросы идут внахлёст, и «хвост журнала» одного включит записи
        другого. Приёмник живёт в контексте выполнения; вложенные прогоны
    ...
    def total_cost_rub(self) -> float:
        # Не до копеек: из коротких вызовов складывается счёт за поток работ.
