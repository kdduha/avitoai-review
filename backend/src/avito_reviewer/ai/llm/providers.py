...
OpenAI-совместимом протоколе: клиент один, различаются `base_url` и ключ.
...
который ходит наружу с чужими данными, — лишняя поверхность.
"""
    ...
    def complete(
        ...
        *,
        ...
        if isinstance(text, Exception):  # позволяет проверять обработку сбоев
            raise text
        # Отвечает запрошенной моделью, как настоящий провайдер: на расхождении
        # двойника прячутся ошибки роутинга.
...
def provider_from_config(config: LLMConfig) -> Provider:
    ...
    if config.provider == "external":
        ...
        # падаем здесь, а не 401-м на первом же ревью.
