...
только в пределах своего бюджета, всё крупное едет diff-only, а тело живёт за
`content_ref`. Этот модуль — то место, где из трёх разных источников
получается один объект, с которым работают и ревью, и детектор.

Главное здесь не резолвер, а флаг `partial`. Вердикт «в работе нет обработки
ошибок», сделанный по одному диффу, — некорректный вердикт: мы не видели
файла целиком. Поэтому неполнота не прячется:

* в промпте такой файл помечен, и системная инструкция запрещает делать по
  нему выводы об отсутствии чего-либо;
* валидатор цитат говорит «не найдено в доступной части», а не «такого текста
  нет» — соврать и не увидеть это разные вещи;
* стилометрия и перплексия такие файлы пропускают: по обрывкам строк
  статистика даёт и ложную ровность, и ложную рваность;
* список неполных файлов уходит в черновик и в ограничения детектора.

Отдельный случай — файл, добавленный этой сдачей: его дифф и есть весь файл,
поэтому текст из диффа полон, а номера строк точны. Проверяем это по покрытию
строк и по размеру из дерева, а не по статусу: дифф крупного файла GitHub
может и обрезать.
"""
...
# набирает почти весь размер из дерева. Допуск — на перевод строки в конце и
# на расхождение в кодировке.
_SIZE_TOLERANCE = 0.9
...
async def _fetch_bodies(
    ...
    # По возрастанию размера: за тот же бюджет получаем больше целых файлов, а
    # самый толстый и без того чаще всего не нужен целиком.
