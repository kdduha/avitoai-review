...
Самый весомый сигнал ансамбля и единственный полностью детерминированный: он
смотрит не на текст, а на то, как работа появлялась во времени. Объяснимость
здесь важнее точности — сигнал рекомендательный, и ревьюер должен видеть
основания, а не число.

Что ищем: взрывную вставку (640 строк одним коммитом за три минуты при среднем
коммите на 40), единственный коммит на всю работу, неправдоподобный темп,
механические интервалы между коммитами. Пустая история — не нулевой скор, а
«сигнал недоступен».

**Чего мы не знаем.** `Revision` не несёт списка затронутых файлов, поэтому
сказать «эти строки появились в том коммите» нельзя. Спаны строятся от
артефактов — подсвечиваются крупные файлы, добавленные сдачей, — и в
обосновании прямо написано, что связь косвенная.
"""
...
def analyse(bundle: SubmissionBundle, weight: float = 0.35) -> SignalResult:
    ...
    # 2. Механический ритм. Только короткие промежутки: коммиты каждый вечер —
    #    распорядок дня, а через равные полторы минуты — выкладывание готового.
