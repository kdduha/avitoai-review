...
Сигнал опциональный: требует локального сервинга с logprobs. Наружу не уходит
никогда — внешние провайдеры логпробы почти не отдают, объём токенов велик, а
стоить он должен ноль.

По скользящему окну меряются средняя неожиданность (сгенерированный текст
модель предсказывает слишком хорошо) и burstiness — разброс между соседними
окнами. Второе важнее: низкая перплексия бывает и у формального текста
человека, ровная низкая на протяжении всей работы — признак специфичный.

Без локального провайдера сигнал сообщает, что недоступен, и ансамбль
перераспределяет веса: отсутствие сигнала и отсутствие признаков — разные вещи.
По той же причине пропускаются файлы, доступные фрагментом: окно по склейке
обрывков меряет наши разрывы, а не текст студента.
"""
