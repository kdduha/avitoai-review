...
аккуратный студент выглядит для него как модель. Отсюда небольшой вес и
требование: каждое срабатывание несёт объяснение, чтобы ревьюер отклонил его
одним взглядом.

Для текста работает прокси-мера burstiness: у человека длина предложений
скачет, у сгенерированного текста она ровная. Настоящая перплексия делает это
лучше, но требует локальной модели с logprobs.

Файлы, доступные фрагментом, пропускаются целиком: контекстные строки вокруг
изменений дают ложную ровность, разрывы между ханками — ложную рваность.
"""
...
CODE_LANGS = frozenset(
    {"go", "python", "javascript", "typescript", "java", "rust", "c", "cpp", "kotlin",
...
def analyse(texts: list[ArtifactText], weight: float = 0.15) -> SignalResult:
    result = SignalResult(kind=SignalKind.STYLOMETRY, weight=weight)
    usable = [text for text in texts if not text.partial]
    ...
    if not scores:
        ...
        if not usable:
            ...
            result.note = (
                "нет файлов, доступных целиком: " + result.note
                if result.note
                else "нет артефактов для анализа"
            )
        return result

    # Взвешенное к максимуму среднее, а не максимум: один подозрительный файл из
    # двадцати — ещё не характеристика работы.
...
def _is_code(artifact: ArtifactText) -> bool:
    ...
    return artifact.path.lower().endswith(CODE_SUFFIXES)


def _code_signals(artifact: ArtifactText) -> tuple[float, list[str]]:
...
def _text_signals(artifact: ArtifactText) -> tuple[float, list[str]]:
    text = artifact.text
