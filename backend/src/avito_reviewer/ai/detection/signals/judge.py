...
Единственный сигнал ансамбля, который ходит наружу, и требование то же, что к
ревью-агенту: **цитата обязательна**. Ответ без дословного фрагмента
отбрасывается целиком — иначе показать ревьюеру нечего.

Шаблонный код исключается по allowlist: без него детектор уверенно ловит
`go.sum`, миграции и boilerplate из методички, то есть самого себя.

Файлы, доступные фрагментом, сюда попадают, в отличие от стилометрии: находка
judge — утверждение о конкретном фрагменте, который у нас на руках. Фрагмент
помечен в промпте, чтобы модель не рассуждала о работе целиком по куску файла.
"""
...
def analyse(
    ...
    *,
    ...
    try:
        ...
        # Детектор не должен ронять обработку сдачи: остальные сигналы работают,
        # а ревьюер увидит, что этот недоступен.
...
def _locate(artifact: ArtifactText, finding: JudgeFinding) -> Span | None:
    ...
    # У фрагмента нумерация идёт с пропусками, поэтому строки берём из карты
    # номеров артефакта, а не из позиции в доступном тексте.
    return Span(
