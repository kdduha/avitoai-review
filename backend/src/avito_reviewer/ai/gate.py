...
Условия сами содержат жёсткие формальные требования: «лог должен писать
`Shutting down service-courier`», «порт переопределяется флагом `--port`»,
«код разделён на `cmd/` и `internal/`». Гейт исполняет их до обращения к
модели; результат каждой проверки уходит в промпт фактом, а ревьюеру строкой.
«18 тест-кейсов вместо 21» должно остаться точным числом, а не превратиться в
вероятностный ответ модели.

Гейт сообщает, но не запрещает: статуса `blocked` нет. Раньше непройденная
проверка уровня `blocking` возвращала работу ревьюеру, не запуская модель, и
студент за одну недостающую строку в логе не получал ни слова о том, что
сделано хорошо.

**Чего не видели — то не «отсутствует».** Проверка на присутствие строки в
файле, доступном фрагментом, не может дать отрицательный ответ: она
помечается неубедительной и уходит человеку.
"""
...
def _status(outcomes: list[CheckOutcome]) -> GateStatus:
    # `o.blocks` — уровень важности проверки, а не статус отчёта: по нему
    # отличают дословное требование условия от пожелания.
...
def _run_check(check: FormatCheck, context: GateContext) -> CheckOutcome | None:
    ...
    if handler is None:
        # Часть требований исполняется только другим каналом сдачи (шрифт и
        # число страниц — в Google Docs). Пропавшая проверка означала бы
        # работу, прошедшую гейт без проверки.
...
def _revision_history(check: FormatCheck, context: GateContext) -> CheckOutcome:
    ...
    Требование написано про Google Docs, но в git исполнимо: коммиты лежат в
    бандле — сознательная переинтерпретация во втором канале.

    Пустая история — не «истории нет», а «источник её не отдал»: pull request без
    коммитов не бывает, поэтому ответ уходит человеку. Судить, что история из
    одного коммита подозрительна, — работа форензики детектора, а не гейта.
    """
