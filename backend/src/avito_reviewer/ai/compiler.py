...
Единственное место конвейера, где галлюцинация модели стоит дорого: ошибка в
рубрике тиражируется на каждую работу потока. Поэтому компилятор
**предлагает**, а методист подтверждает результат один раз на задание.

Три правила, показывающие, чему можно верить:

**Каждый критерий несёт цитату из условия, и она сверяется программно** — тот
же механизм, что у цитат в ревью. Нет дословного фрагмента — критерий помечен
как пересказанный и уходит методисту первым.

**Готовые списки переносятся дословно.** Блоки вида «Тест-кейсы (проверочные
пункты для преподавателя)» идут в `checks[]` один в один.

**Чего в условии нет — то не выдумывается.** Если баллов нет вовсе, шкала не
сочиняется, а выносится в `open_questions`.
"""
...
def _assemble(
    ...
    # Одним вопросом, а не по критерию: методисту нужно решение о шкале целиком.
