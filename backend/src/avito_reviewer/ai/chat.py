...
Модель работу заново не оценивает: читает черновик, файлы и рубрику через тулы
и либо отвечает, либо просит очередной тул. `propose_review_patch` возвращает
`ProposedPatch`, а применяет его `PATCH /submissions/{id}/review` — тот же путь,
что и ручная правка, с тем же пересчётом и записью в `review_revisions`. После
предложенного патча цикл останавливается: решение за ревьюером.

Каждый шаг (вызов тула, его результат, финальный ответ) — отдельный `ChatStep`:
роутер персистит и стримит их по одному, а не ждёт весь цикл.
"""
...
def _render_criterion(rubric: Rubric, criterion_id: str, draft: ReviewDraft) -> str:
    ...
    # Цитаты вердикта — здесь, а не отдельным тулом: они уже лежат в этом же
    # черновике, и отдельный тул стоил бы лишнего шага цикла.
