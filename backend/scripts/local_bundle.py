...
Для демо на реальных работах из `homework_examples`, где нет ни PR, ни токена.
Собирается тот же объект, что отдал бы провайдер.

Провайдером намеренно не оформлено: `SubmissionProvider` — контракт для
источников сдач, а каталог на диске им не является.

У снимка каталога нет ни PR, ни base..head: `source` и `origin_url` —
заполнители, а весь файл считается добавленным этой сдачей.
"""
