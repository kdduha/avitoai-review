...
Скрипт, а не запись в таблицу руками, потому что черновик получается настоящим:
тот же гейт, агрегатор и сверка цитат, что у живого прогона. Записаны только
ответы модели, цитаты в них взяты из настоящих строк файла. Готовый черновик,
вписанный в базу, разошёлся бы с кодом при первой правке рубрики.

    uv run python scripts/seed_demo_submission.py

Идемпотентно: сдача с этим origin_url заводится один раз.
"""
...
def full_marks(texts: list[ArtifactText], rubric: Rubric, batch_size: int) -> list[str]:
    ...
    # Итоговый отзыв последним: он пересказывает уже проставленные вердикты.
    out.append(
        json.dumps(
            {
                "strengths": [
...
async def main() -> int:
    ...
    async with sessionmaker() as session:
        ...
        # переключатель ролей. Назначенная на `c-kruglov`, сдача была видна
        # только руководителю — остальным `_load` отдаёт 404.
        reviewer = (
