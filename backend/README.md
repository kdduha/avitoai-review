...
- `POST /review` — `{link, rubric_id | rubric, gate_facts?, condition_text?, with_detection?}` →
  `{bundle, files, draft, detection?}`: сдача, тексты файлов и черновик с проверенными
  цитатами. `with_detection` добавляет отчёт детектора, не сходив за сдачей второй раз
...
одним и тем же. По той же причине ревью и детектор считаются одним прогоном —
`with_detection`: два отдельных вызова не только тратят лимит дважды, но и
расходятся по ревизиям, если студент пушнул между ними. Базы ещё нет, поэтому
каждый прогон — свежий ingest.
...
**Проверки и оценка**
- [x] Format Gate по git-каналу: пути, строки в коде, объём, история ревизий —
      до вызова LLM, без единого токена
- [ ] Format Gate вне git: шрифты, число страниц и тест-кейсов, smoke-тесты.
      Правила уже лежат в рубриках (`format_gate`); проверка без обработчика не
      пропадает, а уходит ревьюеру как непроверенная
