...
RBAC — лестница `student < reviewer < methodist < admin`, каждая ручка требует свой
...
- [x] RBAC: `student < reviewer < methodist < admin`; reviewer видит и правит только свою
      очередь (404 на чужую сдачу, не 403 — не подтверждаем даже её
      существование), admin — весь поток. `methodist` владеет рубриками,
      заданиями и сроками; `admin` — аккаунтами, переназначением и деньгами.
      Курсы, потоки и задания в API теперь есть, скоуп по ним — по зачислению
      студента и по назначению ревьюера на поток
...
- [x] Format Gate — статические проверки рубрики по git-каналу
      ...
      `token_budget`, `revision_history_visible`), до вызова LLM. Гейт сообщает,
      но не запрещает: статуса `blocked` нет, результат каждой проверки уходит
      в модель фактом и ревьюеру строкой, а разбор идёт в любом случае
