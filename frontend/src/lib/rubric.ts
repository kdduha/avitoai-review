/** Рубрика на стороне редактора: повтор серверных проверок, заготовки пустых
 *  сущностей и справочник видов проверок Format Gate.
 *
 *  Повтор здесь — не подсчёт, а список поломок, и он нужен методисту в момент
 *  правки, а не после отказа сервера. Правила и формулировки скопированы из
 *  `backend/src/avito_reviewer/ai/rubric.py` (`validate_rubric`), чтобы одна
 *  поломка не называлась на экране и в ответе сервера по-разному. **Источник
 *  правды — сервер**: его 422 показывается как есть.
 */
 ...
 *
 *  Отдельно от `rubricProblems`: те повторяют правила сервера и блокируют
 *  отправку, а эти — вопрос вкуса и решаются методистом. */
