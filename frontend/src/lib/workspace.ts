 ...
 *
 *  Ни один ответ бэкенда сам по себе не полон: вердикт знает балл, но не название
 *  критерия и его максимум (это рубрика), текст файла может начинаться не с
 *  первой строки, спаны детектора живут в отдельном ответе. Сведение здесь
 *  избавляет компоненты от знания о том, кто что хранит.
 */
 ...
 *
 *  Итог не пересчитывается: у агрегатора есть веса, шаг шкалы, обязательные
 *  минимумы и штраф за просрочку, а два последних могут обнулить работу целиком.
 *  Досчитать это здесь значит показать правдоподобное, но неверное число. Поэтому
 *  только сумма по критериям и пометка `edited`; живой прогон берёт пересчитанный
 *  итог с сервера через `withPatchedDraft`.
 */
