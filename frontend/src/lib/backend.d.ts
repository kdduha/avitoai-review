...
export interface components {
    schemas: {
        ...
        ReviewRequest: {
            ...
            /**
             * With Detection
             * @description Вернуть рядом с черновиком отчёт детектора ГенИИ. Это один прогон: бандл и тексты общие, поэтому спаны детектора описывают ту же ревизию, что и цитаты черновика, а источник опрашивается один раз.
             * @default false
             */
        ...
        ReviewResponse: {
            ...
            /** @description Отчёт детектора ГенИИ, если его просили через `with_detection`. `null` значит «не просили»; если просили, но сказать нечего, приходит отчёт с причиной в `limitations`. */
