 ...
 *
 *  Работа — `GO/Слабое решение 1-3/course-go-avito-israpilovsha-af0d8d0…`, код
 *  перенесён из репозитория без правок. Прогон `demo` показывает хорошую работу,
 *  этот — вторую половину картины.
 *
 *  Что показывает этот прогон и не показывает ни один другой:
 *
 *  - **обязательные минимумы агрегатора**: 7 из 10 при пороге 6 — и всё равно
 *    незачёт, потому что критерий «Корректное завершение по SIGINT/SIGTERM»
 *    получил 0.5 при минимуме 1. Балл там стоит за настоящую ошибку:
 *    `srv.Start()` возвращает `http.ErrServerClosed`, код сравнивает его с
 *    `context.Canceled` и уходит в `log.Fatalf`, не дождавшись `Shutdown`;
 *  - **`inconclusive` в Format Gate на файле с разрывом нумерации.**
 *    `internal/courier/repository/postgres.go` приехал ханками диффа (строки
 *    20–30 и 63–78) и помечен `partial`, поэтому проверка на строку в логе
 *    возвращается вопросом: `gate._code_contains` запрещает отрицательный ответ,
 *    если среди кандидатов есть фрагмент, и не различает, в каком файле искали.
 *    Модель видит `cmd/myapp/main.go` целиком и говорит про сообщение прямо в
 *    вердикте по c4 — гейт грубее модели.
 *
 *  Семь проверок гейта из девяти пройдены. Провала по отсутствующему
 *  `migrations/` в гейте намеренно нет: `required_paths` рубрики `go-task1`
 *  перечисляет только `cmd/` и `internal/`, миграции требует условие задания 2.
 *  Факт вынесен в вердикт по c1 и в причины внимания.
 *
 *  `student_ref.internal_id` обезличен шестизначным числом; в путях импорта логин
 *  остаётся — код показан ревьюеру как есть, вычищает его скрабер шлюза.
 */
