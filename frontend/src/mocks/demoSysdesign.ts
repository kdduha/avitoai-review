 ...
 *
 *  Работа набирает ровно порог — 4 из 6 — и всё равно получает незачёт:
 *  у критерия c5 в рубрике стоит `min_score_for_pass: 1`, а построена только
 *  контекстная диаграмма C4 из двух требуемых. Провал обязательного минимума
 *  агрегатор не компенсирует набранным на остальных критериях.
 *
 *  Раньше здесь стояло `passed: true` с объяснением «работа набрала ровно
 *  порог». Это расходилось с самим бэкендом: `ai/review/aggregate.py` при
 *  непустом `failed_minimums` возвращает незачёт независимо от суммы. Прогон
 *  лежит в формате ответов сервера именно затем, чтобы такое расхождение
 *  всплывало здесь, а не на живом ключе.
 */
...
export const DEMO_SYSDESIGN_REVIEW: ReviewResponse = {
  ...
  draft: {
    ...
    passed: false,
    pass_explanation:
      'не набран обязательный минимум: c5 «Построена диаграмма C4 (контекстная и компонентная)»: 0.5 при обязательном минимуме 1',
