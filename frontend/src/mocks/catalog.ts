...
/** Кто какие программы ведёт. Все id обязаны быть из `PROGRAMS`: `KNOWN_COURSES`
 *  ниже отфильтровывает ссылку на несуществующий курс молча, и дефект выглядит
 *  не падением, а незанятым человеком с нулевой загрузкой.
 *
 *  Распределены по навыкам из карточек `backend/reviewers/`. Лев Гринёв оставлен
 *  без назначений намеренно — руководителю есть кого распределять. */
