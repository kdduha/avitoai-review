 ...
 *  `analytics`, `fraud`, `business-models`), и четыре куратора из десяти
...
function buildGrades(): Grade[] {
  ...
  for (const student of STUDENTS) {
    ...
    for (const assignment of ASSIGNMENTS.filter((item) => item.courseId === student.courseId)) {
      ...
      /* Часть черновиков куратор утверждает как есть — по этим строкам потом
         считается «принято без правок»; доля утверждений без правки —
         ОЦЕНКА, а не измерение. */
      const acceptedAsIs = next() < 0.46
      /* Если куратор правит балл, он двигает его на шаг-другой шкалы: правка
...
export function statsForStream(streamId: string): StreamStats | null {
  ...
  /* ОЦЕНКА: очередь делится между кураторами поровну — настоящего назначения
     на конкретную сдачу у нас нет. Минуты считаются по медиане куратора, и
     она тоже оценочная (`CURATOR_SEED`). Зато сумма по кураторам равна
  ...
  /* Куратор подтверждает сильный сигнал и отклоняет слабый — ПРАВИЛО, а не
  ...
  /* «Принято без правок» теперь считается: это утверждённые работы, у
     которых балл куратора совпал с предложением модели. */
  ...
  return {
    ...
    /* Шаги считаются по статусам, а не по коэффициентам: черновик собирается
       для каждой сдачи, дальше работа уходит к куратору и получает оценку. */
    funnel: [
      { stage: 'Сдано', count: submitted },
      { stage: 'Черновик готов', count: submitted },
      { stage: 'Взял куратор', count: submitted - draftReady },
