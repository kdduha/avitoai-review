 ...
 *  Колонки: ID | Итоговая сумма | Посещаемость | Вовлеченность | Количество дз |
 *  Экзамен | Итог | Оценка | Согласие на оценку.
 *
 *  Проверено арифметикой на всех девяти строках:
 *
 *  1. `итог = сумма × 0.5 + экзамен × 0.4 + вовлечённость / 10`; посещаемость в
 *     формулу не входит.
 *  2. Вовлечённость принимает ровно три значения: 0, 10, 20.
 *  3. Экзамен у всех девяти — 30: это максимум, а не совпадение.
 *  4. Оценка выводится по таблице абсолютных порогов (`MARK_TABLE`). Пороги в
 *     долях от максимума воспроизводят только четыре строки из девяти.
 *  5. В «Количестве дз» у одной строки стоит «ю» — настоящая опечатка, отсюда
 *     тип `number | string`.
 *  6. «Согласие на оценку» — свободный текст: пять написаний на девять строк.
 *
 *  Курс состоял из девяти домашних работ, и «Итоговая сумма» ведётся на его
 *  шкале — отсюда `LEDGER_HOMEWORK_MAX`.
 */
