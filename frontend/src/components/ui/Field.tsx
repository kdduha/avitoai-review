 ...
 *
 *  Разобранное значение нельзя писать обратно в поле на каждое нажатие: «0.» и
 *  «-» — законные промежуточные состояния ввода, а `Number('0.')` даёт 0 и
 *  стирает точку из-под курсора. Поэтому пока поле в фокусе, текст принадлежит
 *  пользователю, наружу уходит только то, что разобралось, а на выходе из поля
 *  текст приводится к значению.
 */
